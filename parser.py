...
class Scene:
    ...
    def print_info(self):
        ...
        print(f"Unique Characters: {self.unique_characters}")
        print(f"Dialogue Count: {self.dialogue_count}")
        print(f"Estimated Duration: {self.estimated_duration} seconds\n")
...
class Script:
    ...
    def parse_script(self, script):
        ...
        for line in script.splitlines():
            ...
            match = re.match(r'^(?:INT\.|EXT\.)\s*(.*)$', stripped_line)
        
            if match:
                if scene_number > 0:
                    scene = Scene(scene_heading, actions, dialogues, scene_number) #instantiating scene object
                    self.scenes.append(scene) #appending scene objects to scene list
